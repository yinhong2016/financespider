...
/**
 * @Description: 实盘股票组合持仓记录装配器
 */
public class ActualCubeLastBalancingMapper extends AbstractMapper<CubeEntity, CubeEntity> {
  ...
  public ActualCubeLastBalancingMapper() {
    this(COUNT_DEFAULT);
  }

  public ActualCubeLastBalancingMapper(int count) {
    super();
    if (count <= 0) {
  ...
  public CubeEntity mapLogic(CubeEntity cube) {
    ...
    // 雪球组合历史持仓股票记录
    String target = URLMapper.XQ_ACTUAL_CUBE_REBALANCING_JSON.toString();
  ...
  private void processCube(CubeEntity cube, JsonNode node) {
    ...
    for (JsonNode jsonNode : list) {
      for (JsonNode jn : jsonNode.get("rebalancing_histories")) {
        ...
        String prev_weight = jn.get("prev_weight_adjusted").asText();
        // String target_weight = jn.get("target_weight").asText();
        String weight = jn.get("target_weight").asText();
        String rebalancing_id = jn.get("id").asText();
        RebalancingEntity.TrendBlock trendBlock = new RebalancingEntity.TrendBlock(stock_name,
            stock_symbol, "", "", price, prev_weight, "", weight, rebalancing_id);
